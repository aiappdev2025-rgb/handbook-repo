---
sidebar_position: 5
title: "Competitive Analysis"
description: "Analyze competitors systematically to find differentiation angles and inform your pricing strategy"
---

import {
  TldrBox,
  WhenToUse,
  Prerequisites,
  ExpectedOutput,
  Verification,
  NextSteps,
  ChapterNav
} from '@site/src/components/ChapterComponents';

# Competitive Analysis

<span className="reading-time">20 min read</span>

<TldrBox
  what="Deep-dive into 5-10 competitors to understand their positioning, pricing, strengths, weaknesses, and gaps you can exploit."
  why="You can't differentiate if you don't know what you're differentiating from—competitive analysis reveals your strategic opportunities."
  outcome="A competitive analysis document with positioning map, feature comparison, and clear differentiation strategy."
/>

<WhenToUse
  useWhen={[
    "You know your market but not your competitors in depth",
    "You need to find differentiation angles",
    "Your pricing strategy is undefined"
  ]}
  notYetIf={[
    "Business One-Pager not complete",
    "You haven't defined your target customer"
  ]}
  skipIf="You're operating in a brand new market with no competitors (rare)"
/>

<Prerequisites
  items={[
    { text: "Business One-Pager complete", link: "/phase-1-validate/business-one-pager" },
    { text: "Target customer defined" }
  ]}
/>

## Why Deep Competitive Analysis?

Your market research identified competitors. Now you need to understand them deeply enough to:
- Find gaps they're not serving
- Position yourself differently
- Price competitively
- Learn from their mistakes

## The Analysis Framework

### 1. Competitor Identification

List all competitors in three tiers:

| Tier | Description | Examples |
|------|-------------|----------|
| **Direct** | Same solution, same customer | Your closest competitors |
| **Indirect** | Different solution, same problem | Alternative approaches |
| **Potential** | Could enter market easily | Adjacent products, big players |

### 2. Per-Competitor Deep Dive

For each direct competitor (5-10 total), document:

#### Basic Information
- Company name and URL
- Founded date
- Funding (if known)
- Team size (if known)

#### Product Analysis
- Core features
- Pricing model and tiers
- Target customer segment
- Unique selling proposition

#### Strengths & Weaknesses
- What do they do well?
- Where do they fall short?
- What do customers complain about? (Check reviews, forums, social media)

#### Market Position
- Premium vs budget
- Enterprise vs SMB
- Generalist vs specialist

### 3. Feature Comparison Matrix

Create a comparison of key features:

| Feature | Your Product | Competitor A | Competitor B | Competitor C |
|---------|--------------|--------------|--------------|--------------|
| Feature 1 | ✅ | ✅ | ❌ | ✅ |
| Feature 2 | ✅ | ❌ | ✅ | ❌ |
| Feature 3 | ✅ | ✅ | ✅ | ✅ |
| Price | $X/mo | $Y/mo | $Z/mo | $W/mo |

### 4. Positioning Map

Plot competitors on two dimensions that matter to your customers:

```
High Price
    │
    │    ○ Enterprise Player
    │
    │         ○ Premium Tool
    │
────┼──────────────────────────
    │    ★ YOUR POSITION
    │
    │  ○ Budget Option
    │
Low Price

    Simple ←────────────→ Complex
```

## Competitive Analysis Prompt

```markdown
Help me create a competitive analysis for my SaaS product.

**My Product**: [DESCRIBE YOUR PRODUCT]
**Target Customer**: [FROM ONE-PAGER]

For each competitor, analyze:

1. **Competitor List**: Identify 5-10 direct competitors
2. **Deep Dive**: For each competitor:
   - Core features and pricing
   - Target customer segment
   - Strengths and weaknesses
   - Customer complaints (search reviews, G2, Capterra, Reddit)
3. **Feature Comparison**: Matrix of key features vs competitors
4. **Positioning Map**: Where does each competitor sit on price vs complexity?
5. **Gap Analysis**: What opportunities exist that competitors aren't serving?
6. **Differentiation Strategy**: How should we position against them?

Output in markdown format, ready to save as docs/competitive-analysis.md.
```

## Finding Competitor Weaknesses

The best differentiation comes from competitor weaknesses. Look for:

| Weakness Type | What to Look For |
|---------------|------------------|
| **Feature gaps** | "I wish it could..." reviews |
| **Price complaints** | "Too expensive for what you get" |
| **Complexity issues** | "Hard to set up", "Steep learning curve" |
| **Support problems** | "Never responds to tickets" |
| **Performance issues** | "Slow", "Crashes", "Unreliable" |
| **Integration gaps** | "Doesn't work with [tool I use]" |

:::tip Review Mining
G2, Capterra, Product Hunt, and Reddit are goldmines for competitor weaknesses. Search "[Competitor] complaints" or "[Competitor] alternative" to find pain points.
:::

## Differentiation Strategies

Choose one primary differentiation:

| Strategy | Description | Example |
|----------|-------------|---------|
| **Price** | Significantly cheaper | "Same features, half the price" |
| **Simplicity** | Easier to use | "Set up in 5 minutes, not 5 hours" |
| **Niche** | Serve a specific segment better | "Built specifically for [role]" |
| **Feature** | Unique capability | "The only tool that does X" |
| **Integration** | Better ecosystem fit | "Native integration with [popular tool]" |
| **Service** | Superior support | "White-glove onboarding included" |

<ExpectedOutput
  items={[
    { type: "Artifact", description: "`docs/competitive-analysis.md`" },
    { type: "Analysis", description: "5-10 competitors with deep dives" },
    { type: "Matrix", description: "Feature comparison table" },
    { type: "Map", description: "Positioning map showing market gaps" },
    { type: "Strategy", description: "Clear differentiation approach" }
  ]}
/>

<Verification
  checks={[
    "At least 5 direct competitors analyzed in depth",
    "Pricing for each competitor documented",
    "Customer complaints/weaknesses identified (from real reviews)",
    "Feature comparison matrix created",
    "Positioning map shows your intended position",
    "Clear differentiation strategy chosen",
    "Document saved to repository"
  ]}
/>

<NextSteps
  next={{
    chapter: 6,
    title: "MVP Scoping",
    path: "/phase-1-validate/mvp-scoping"
  }}
  previous={{
    chapter: 4,
    title: "Business One-Pager",
    path: "/phase-1-validate/business-one-pager"
  }}
  related={[
    { title: "Market Research", path: "/phase-1-validate/market-research" },
    { title: "Design Brief", path: "/phase-1-validate/design-brief" }
  ]}
/>

<ChapterNav
  current={5}
  total={43}
  previous={{
    chapter: 4,
    title: "Business One-Pager",
    path: "/phase-1-validate/business-one-pager"
  }}
  next={{
    chapter: 6,
    title: "MVP Scoping",
    path: "/phase-1-validate/mvp-scoping"
  }}
/>
