...
description: "Track and manage technical debt across milestones with structured checkpoint audits"
---
...
<TldrBox
  what="Templates for assessing and tracking technical debt at each checkpoint in your development process."
  why="Technical debt accumulates silently. Regular audits surface issues before they become blockers, letting you make informed decisions about when to address debt."
  outcome="A TECH-DEBT.md file tracking debt status and remediation plans at each checkpoint."
/>

<WhenToUse
  useWhen={[
    "Post-milestone cleanup and refactoring sessions",
    "At Checkpoint A (after M5/Authentication)",
    "At Checkpoint B (after M6/Core Features)",
    "At Checkpoint C (before production launch)"
  ]}
  notYetIf={[
    "You're in the middle of implementing a feature",
    "You haven't reached a checkpoint yet"
  ]}
  skipIf="You're building a throwaway prototype with no intention to maintain"
/>

## What is Technical Debt?

Technical debt is the implied cost of future rework caused by choosing an easy solution now instead of a better approach that would take longer. It includes:

- **Code complexity** - Functions too long, deep nesting
- **Missing tests** - Untested code paths
- **Type safety gaps** - `any` types, missing validations
- **Documentation drift** - Docs that don't match code
- **Dependency issues** - Outdated or vulnerable packages
- **Security gaps** - Missing validation, weak policies

:::info Debt is Normal
Some debt is intentional (shipping faster). The key is **tracking it** so you can pay it down systematically.
:::

## Template

<TemplateDownload
  name="TECH-DEBT-AUDIT.md"
  file="checkpoint-debt-audit.md"
  description="Complete checkpoint audit templates with scoring system"
/>
...
| Checkpoint | Milestone | Focus Areas |
|------------|-----------|-------------|
| **A** | After M5 (Auth) | Schema, auth, infrastructure, organization |
| **B** | After M6 (Core) | Complexity, errors, types, tests, docs |
| **C** | After M11 (Pre-launch) | Security, performance, dependencies, monitoring |

## Scoring System

The weighted scoring helps you make informed decisions:

| Total Score | Decision |
|-------------|----------|
| **7.0+** | PASS - Proceed confidently |
| **6.0-6.9** | CONDITIONAL - Proceed with remediation plan |
| **<6.0** | BLOCK - Address critical issues first |

## Best Practices

### Track Everything
Create a `TECH-DEBT.md` file in your repo. Every audit entry goes here with dates and scores.

### Link to Issues
When you find violations, create tickets. Reference them in the audit:
```markdown
- Refactor `processPayment()` - too complex (tracked in #123)
```

### Schedule Debt Sprints
Allocate time between milestones to address accumulated debt. A 7.0 score can become 6.0 if ignored.

### Be Honest
It's tempting to inflate scores. Accurate audits help you make better decisions about when to ship vs. when to fix.
...
- [ ] Violations documented with specifics
- [ ] Score calculated correctly
- [ ] Gate decision made and documented
- [ ] Action items have issue numbers
- [ ] TECH-DEBT.md updated in repo
