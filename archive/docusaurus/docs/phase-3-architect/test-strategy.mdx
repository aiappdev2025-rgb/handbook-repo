...
<span className="reading-time">15-20 min</span>

<TldrBox
  what="Define what you'll test, how you'll test it, and what tools you'll use. A clear test strategy prevents bugs from reaching production and gives you confidence to ship fast."
...
<Prerequisites
  items={[
    ...
    { text: "Tech stack decisions made" }
  ]}
/>
...
- `docs/test-strategy.md` — Your testing plan
...
<SmartPrompt
  ...
  template={`Create a test strategy for [PRODUCT_NAME].
...
<ExpectedOutput
  items={[
    { type: "Strategy Doc", description: "`docs/test-strategy.md` with coverage targets and test structure" },
    { type: "Configuration", description: "`vitest.config.ts` ready to run tests" },
    { type: "Setup File", description: "`src/test/setup.ts` with environment mocks" },
    { type: "Example Test", description: "`src/lib/__tests__/example.test.ts` demonstrating patterns" },
    { type: "Scripts", description: "Test scripts added to `package.json`" }
  ]}
/>

<Verification
  checks={[
    "`docs/test-strategy.md` created by Claude Code",
    "`vitest.config.ts` created with proper configuration",
    "`src/test/setup.ts` created with environment mocks",
    "Example test file created in `src/lib/__tests__/`",
    "Test dependencies installed",
    "`npm run test` passes with example tests"
  ]}
/>
